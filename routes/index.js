...
router.get('/addadmin', async function (req, res) {
  const token = req.cookies.jwt;
  res.render("addadmin", { Title: "Thêm Quần Áo", token })
})
...
router.get('/timquanaoadmin', async function (req, res) {
  const token = req.cookies.jwt;

  try {
    const query = req.query.query;
    const response = await axios.get(`http://localhost:3000/api/search_quanaoadmin?name=${query}`, {
      headers: { Authorization: `jwt ${token}` },
    });
    const data = response.data;
    res.render("quanlyquanaoadmin", {
...
router.get('/quanao/:id', async function(req, res) {
  ...
  try {
    ...
    console.log("đức dau moi",quanaos.toJSON());
...
router.get("/", function (req, res, next) {
  const token = req.cookies.jwt;
  console.log("đức token hi", token)
  if (token) {
