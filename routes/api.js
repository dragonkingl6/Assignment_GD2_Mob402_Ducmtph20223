...
router.get('/search_quanaoadmin', passport.authenticate('jwt', { session: false }), async function (req, res) {
    var token = getToken(req.headers);
    if (token) {
        let name = req.query.name; // extract the 'name' query parameter
        let regex = new RegExp(name, 'i'); // create a case-insensitive regular expression
...
router.post("/quanaoadmin", passport.authenticate("jwt", { session: false }), function (req, res) {
    var token = getToken(req.headers);
    if (token) {
        console.log(req.body);
        var newquanao = new quanao({
        ...
        newquanao
            .save()
            .then(() => res.redirect("/quanao/admin"))
            .catch((err) => res.json({ success: false, msg: "Save quanao failed." }));
    } else {
        return res.status(403).send({ success: false, msg: "Unauthorized." });
    }
